...
export class JssGraphQLService {
  ...
  private addJssAmbientVariables<V = R>(query: DocumentNode, variables: OperationVariables, rendering?: ComponentRendering) {
    if (!variables) {
      variables = {} as V;
    }
   ...
   */
  query<T, V = R>(options: QueryOptions<OperationVariables> & JssGraphQLOptions): Observable<ApolloQueryResult<T>> {
   ...
   */
  mutate<T, V = R>(options: MutationOptions<V> & JssGraphQLOptions): Observable<FetchResult<V>> {
    if (this.isEditingOrPreviewingAndSsr) {
      return empty();
    }
